...
import com.migueljuncalp.orderservice.domain.InsufficientStockException;
...
public class KafkaConfig {
    ...
    NewTopic stockMovementsTopic() {
        return TopicBuilder.name(STOCK_ORDERS_TOPIC).partitions(3).replicas(1).build();
    }

    @Bean
    NewTopic stockMovementsDltTopic() {
        return TopicBuilder.name(STOCK_ORDERS_TOPIC + "-dlt").partitions(3).replicas(1).build();
    }
